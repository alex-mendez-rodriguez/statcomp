...
We also load auxiliary routines stored at `routines.R` (directory
`code`)
...
Meaning that it's important to check whether the model fits the main
...
-   `glm` fits generalized linear models (for non-normal outcomes, e.g.
    binary)
...
$$\hat{\beta} \sim N(\beta, V); V= \sigma^2 (X^T X)^{-1}$$ Which gives
confidence intervals and P-values, e.g.
...
**Result.** Let $Z \sim N(\mu, V)$ be a $p$-dimensional Normal
distribution, and $C$ a $q \times p$ matrix with full rank $q \leq p$.
Then $$
...
-1 & 1 & 0 & \ldots & 0 \\
\end{pmatrix}$ Then
$$C \begin{pmatrix} \hat{\beta}_1 \\ \hat{\beta}_2 \\ \ldots \\ \hat{\beta}_p \end{pmatrix} = \hat{\beta}_2 - \hat{\beta}_1 \sim N(\beta_2 - \beta_1, \sigma^2 C (X^T X)^{-1} C^T)$$

## Example. Diamonds data

What drives diamond prices? Exploration suggests that low-quality
diamonds are more expensive (worst diamond color is J (yellow-ish),
worst clarity is l1)
...
log2 facilitates interpretation (+1 in log2 scale $\Rightarrow \times 2$
in original scale)
...
Second model has better $R^2$ coefficient. Careful though, comparison
not fully reliable due to over-fitting (to be discussed)
...
`summary` and `confint` give $\hat{\beta}_j$, P-values for
$H_0:\beta_j=0$ and confidence intervals
...
Easier to define a function that summarizes any fitted model. I created
function `coefSummary` (at `routines.R`) for that purpose
...
Note: [tidymodels](https://www.tidymodels.org) and
[parsnip](https://parsnip.tidymodels.org) provide a unified interface
for many statistical/machine learning models

Note: R package `caret` does as well (see an
[introduction](https://topepo.github.io/caret))
...
-   Error normality (if $n$ large, only important for predictive
    intervals)
...
Note: qq-normal plots require that residuals have 0 mean, variance=1
(function `scale`)
...
FiveThirtyEight's
[post](https://fivethirtyeight.com/features/higher-rates-of-hate-crimes-are-tied-to-income-inequality)
on hate crimes around the 2016 USA elections

Dataset `hate_crimes` (package `fivethirtyeight`) has this info for each
state:

-   `avg_hatecrimes_per_100k_fbi`: pre-election number of hate crimes
    per 100,000 population by the FBI
...
Plus variables about racial diversity, economics etc (see
`help(hate_crimes)`)

------------------------------------------------------------------------

Load the data, select and rename the needed variables, and remove Hawaii
...
1.  Fit a linear regression for `hatecrimes_fbi` vs. `votes_trump`,
    `gini`, `hs`. Report the point estimate, 95% intervals and P-values
    for the 3 covariates. Which of them are significantly associated to
    hate crimes?

2.  Assess the model assumptions: linearity, constant error variance,
    error normality. To what extent are they violated?

Turn in an html with your solution at Aula Global Name your file
...
To interpret the coefficients of categorical variables, we must
understand how they're coded
...
The predicted difference cut - fair diamond is hence $\beta_2$,
regardless of carats. To facilitate interpretation, since
$\mbox{lprice}_i$ is $\log_2$ price,
...
Estimates and CIs in the log2-scale (recall: `coefSummary` is a function
that we defined)
...
Let's express them in the original scale. Fair -\> Good increases price
by 1.08 (i.e. by 8%), Fair -\> Very good by 1.12 (i.e. by 12%), etc.
...
-   Unspecified predictors set to mean (continuous) or reference
    category (discrete)
...
`add_predictions` requires that `model` is of type `lm` / object that
...
Linear contrasts done by `glht` (general linear hypotheses, R package
`multcomp`)
...
Recall that $\hat{\beta} \sim N(\beta, V)$, where
$V= \sigma^2 (X^T X)^{-1}$
...
Recall that $C \hat{\beta} \sim N(C \beta, C V C^T)$. For us
$C \hat{\beta}= \hat{\beta}_6 - \hat{\beta}_5$,
$C\beta =\beta_6 - \beta_5$.
...
Let's fit a model with interactions. All coefficients are statistically
significant
...
Consider all possible interactions. All P-values are highly
...
-   Earlier we plotted price vs. carat & cut. `carat:cut` impacted
    predictions

-   Regressing price vs. carat, cut, color & clarity. \<interactions
    don't change predictions much

Why? After adding all covariates, estimated interaction coef. became
smaller
...
`multiplot` visually compares coefficients across models. Interactions
lcarat:cut are smaller under the full model
...
A particular type of interaction is when data are nested within units
(e.g. countries), and we wanna fit a model for each unit
...
-   `lm` plus interaction with country. Check which is the reference
    country, recover intercept/slopes for the rest

-   Use `group_by` and `map_df`

Note: better alternatives to fit nested models (random
effects/hierarchical models)

First, group by country and `nest` dataset: each row is a country, and
column `$data` is a list of tibbles
...
Finally, use `map_df` to apply `fitmodel` to each entry of the list
`gm_country$data`
...
The $R^2$ coefficient is large for most countries, but pretty small for
a few
...
Select 3 countries, create factor `country` with 3 levels. Create
`years_since_1952`
...
-   For Italy:
    $E(y_i)= \beta_0 + \beta_2 + \beta_1 (\mbox{year}_i - 1952)$
...
Results for $\beta_1$ (years) as before. Different for
$(\hat{\beta}_0, \hat{\beta}_2, \hat{\beta}_3)$
...
-   For Spain:
    $E(y_i)= \beta_0 - \beta_2 - \beta_3 + \beta_1 (x_{i1} - 1952)$

-   Average across countries:
    $E(y_i)= \beta_0 + \beta_1 (x_{i1} - 1952)$

$\beta_0$ is the mean life exp. at 1952 across countries

$\beta_2$ is France's deviation from that mean. $\beta_3$ is Italy's.
$-\beta_2 - \beta_3$ is Spain's

## Exercise

Fit a model with interactions with the 0/1 coding and with the
sum-to-zero coding. Interpret the estimated parameter values from each
model
